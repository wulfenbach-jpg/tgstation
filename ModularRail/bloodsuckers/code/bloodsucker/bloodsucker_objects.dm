...
/obj/item/reagent_containers/blood/attack(mob/living/victim, mob/living/attacker, params)
	...
	if(victim != attacker)
		if(!do_after(victim, attacker, 5 SECONDS))
			return
...
/obj/item/stake
	...
	icon = 'ModularRail/bloodsuckers/icons/stakes.dmi'
	icon_state = "wood"
	inhand_icon_state = "wood"
	lefthand_file = 'ModularRail/bloodsuckers/icons/bs_leftinhand.dmi'
	righthand_file = 'ModularRail/bloodsuckers/icons/bs_rightinhand.dmi'
...
/obj/item/stake/attack(mob/living/target, mob/living/user, params)
	...
	if(!do_after(user, target, staketime, extra_checks = CALLBACK(target, /mob/living/carbon.proc/can_be_staked))) // user / target / time / uninterruptable / show progress bar / extra checks
		return
	// Drop & Embed Stake
	user.visible_message(
